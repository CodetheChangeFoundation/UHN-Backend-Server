...
const validateSignup = () => {
  return [
    ...
    body("phone").exists().bail().withMessage(msg.PHONE_MANDATORY),
  ];
};

const validateLogin = () => {
  return [
    body("username").exists().bail().withMessage(msg.USERNAME_MANDATORY),
    body("password").exists().bail().withMessage(msg.PASSWORD_MANDATORY),
  ];
};

const validateUseRefreshToken = () => {
  return [
    body("userId").exists().bail().withMessage(msg.USER_ID_MANDATORY),
    body("refreshToken")
      .exists()
      .bail()
...
const validateUpdateUserPassword = () => {
  return [body("password").exists().bail().withMessage(msg.PASSWORD_MANDATORY)];
};
