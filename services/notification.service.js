...
const sendBatchNotifications = async (responders, help_request) => {
  let push_tokens = [];
  let notifications = [];
  let user = null;

  try {
    user = await UserService.findUserById(help_request.user_id);
  } catch(err) {
  ...
  for (let pt of push_tokens) {
    ...
    let notification = {
      ...
      data: { user: UserService.cleanUser(user.toJSON()) }
    };
